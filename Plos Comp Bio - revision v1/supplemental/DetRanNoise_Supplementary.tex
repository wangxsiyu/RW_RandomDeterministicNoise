...
\begin{document}
	...
	\subsection{Control analysis: model-free analysis with simulated choice from a random-noise only model}
	\begin{figure}[H]
	\begin{center}
	...
	\end{figure}
	\newpage
	\subsection{Alternative model: fit separate random and deterministic noises for [1 3] and [2 2] conditions}
	...
	\end{figure}
	\newpage
	\subsection{Alternative model: use variance differences as dI}
	Here, we fit a model with an alternative definition of $\Delta I$ that $\Delta I$ is defined to be difference between the variances of rewards shown on the forced-choice trials.
	\begin{figure}[H]
		\begin{center}
	...
	\subsection{Examine the co-variance of random vs deterministic noise}
	\begin{figure}[hp]
		\begin{center}
	...
	In this section, we examined 6 model variants that varied in whether deterministic and random noise are present or not and whether either types of noise is dependent on horizon. Specifically, we tested the following 6 models (Note that the $\sigma^{ran}_{horizon},\sigma^{det}_{horizon}$ model is our original full model).
