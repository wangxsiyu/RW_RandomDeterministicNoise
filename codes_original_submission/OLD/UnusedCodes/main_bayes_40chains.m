...
parfor repi = 1:50
    ...
    ver = 'all';
    versave = 'all_50chains';
    datadir = '/Users/wang/WANG/A_Plate/Wilson_Siyu_RandomDeterministicNoise/data';
    outputdir = fullfile('/Users/wang/WANG/A_Plate/Wilson_Siyu_RandomDeterministicNoise/bayesoutput', versave, '6model');
    mkdir(outputdir);
    nburnin = 2000; % How Many Burn-in Samples?
    nsamples = 1000; % How Many Recorded Samples?
    wj = W_JAGS;
    wj.isdoparallel = false;
    wj.workingdir = sprintf('/Users/wang/Documents/Jags/parfor%d', repi);
    data = importdata(fullfile(datadir,versave,sprintf('bayes_6model_simu_rep%d.mat', repi)));
    for mi = 1:6
        wj.setup_params(1, nburnin, nsamples);
        wj.setup_data_dir(data{mi}, outputdir);
        wjinfo = EEbayes_analysis(data{mi}, 1);
...
for mi = 1:6
    repi = 1;
    tfile = fullfile(outputdir, [sprintf('HBI_DetRanNoise_fitmodel_rep%d_', repi) char(64 + mi) '_samples.mat']);
    samples = importdata(tfile);
    fns = fieldnames(samples);
    for repi = 2:50
        disp(sprintf('%d,%d',mi, repi));
        tfile = fullfile(outputdir, [sprintf('HBI_DetRanNoise_fitmodel_rep%d_', repi) char(64 + mi) '_samples.mat']);
        tsp = importdata(tfile);
        for fi = 1:length(fns)
            fn = fns{fi};
            samples.(fn) = cat(1,samples.(fn), tsp.(fn));
        end
    end
    fff = replace(outputdir, '/6model', '');
    fff = fullfile(fff, [sprintf('HBI_DetRanNoise_fitmodel_') char(64 + mi) '_samples.mat']);
    save(fff, 'samples','-v7.3');
end