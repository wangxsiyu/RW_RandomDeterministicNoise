...
for gi = 1:2
    ...
    plt.setfig(3:4, 'ylim', {[0, 25] + (gi-1)*[10 15],[-8 12]}, ...
        'ytick', {0: 10: 50, -10:5:12});
    plt.ax(1,1);
    plt = EEplot_plm(plt, gp{gi}, 1);
    plt.ax(1,2);
    plt = EEplot_phi(plt, gp{gi});
    plt.ax(2,1);
    plt = EEplot_random(plt, gp{gi}, {'black'}, 'right');
    plt.ax(2,2);
    plt = EEplot_directed(plt, gp{gi}, {'black'}, 'right');
...
% plt.ax(2,1);
% plt.plot(1:2, gp.GPav_p_hi13_h1, gp.GPste_p_hi13_h1, 'line', 'color', 'black');
% plt.plot(1:2, gp.GPav_p_hi13_h2, gp.GPste_p_hi13_h2, 'line', 'color', 'black50');
% plt.setfig_ax('xlabel', 'Horizon', 'xticklabel', {'1', '6'}, 'ylabel', 'p(high info)', 'legend', {'1st half', '2nd half'}, 'xlim', [0.5 2.5], ...
%     'ylim', [.35 .65], 'ytick', 0:.1:1, 'xtick', 1:2,'legloc', 'NW');
% plt.ax(2,2);
plt.ax(1,2)
