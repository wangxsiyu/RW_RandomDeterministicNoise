...
%% dIvar posterior checks



tst = W.load('../bayesoutput_revision/all_revision/HBI_DetRanNoiseR1_dIvar_stat');
st = tst.stats.mean;
simugame = EEsimulate_bayes_2noise_simple(data, st.Infobonus_sub, ...
    st.bias_sub, st.NoiseRan_sub, st.NoiseDet_sub);
simugp = getgpsimu(game0, idxsub, simugame)
plt = W_plt;
plot_posteriorchecks(plt, gp, simugp)