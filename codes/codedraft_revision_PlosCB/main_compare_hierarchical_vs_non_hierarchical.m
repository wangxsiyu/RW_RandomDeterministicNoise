...
for mi = 1:length(modelnames)
    ...
    id2 = randperm(nsub*4);
    st_shuffled = st;
    st_shuffled.NoiseRan_sub = reshape(st.NoiseRan_sub(id1),2,2,[]);
    st_shuffled.NoiseDet_sub = reshape(st.NoiseDet_sub(id2),2,2,[]);
    ...
    save(fullfile(outputdir, ['simugamesMAP' suffix{mi}]), 'simu', 'simu_shuffled', 'st_shuffled')
    %% fit simulated data 
    d = simu;
    d.modelname = sprintf('2noisemodel%s', suffix{mi});
    wj.setup_data_dir(d, outputdir);
...
plt.ax(1);
plt = plot_parameter_recovery(plt, st{1}.stats.mean, st{3}.stats.mean, 3, [1 2], 'hierarchical');
plt.ax(2);
plt = plot_parameter_recovery(plt, st{2}.stats.mean, st{4}.stats.mean, 3,  [3 4], 'non-hierarchical');
plt.update;
    %%
plot_det_vs_ran(plt, st{7}.st_shuffled, st{5}.stats.mean);
...
plot_parameter_recovery(plt, stmap{1}, sp1,1);
plt.update;
plot_parameter_recovery(plt, st{1}.stats.mean, st{3}.stats.mean, 1);
plt.update;
plot_parameter_recovery(plt, stmap{2}, sp2,1);
plt.update;
