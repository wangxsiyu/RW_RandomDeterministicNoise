...
%Human decision making is inherently variable. While this variability was originally seen as suboptimal, recent work suggests that randomness can actually be adaptive. An example arises when we must between exploring unknown options or exploiting options we know well. A little randomness in these `explore-exploit' decisions is remarkably effective, as it encourages to explore options we might otherwise ignore. Moreover, people actually use such `random exploration' in practice,  increasing their behavioral variability when it is more valuable to explore. Despite this progress, it is unknown whether variability for random exploration is generated externally, from stochastic stimuli in the world, or internally, from stochastic processes in the brain. That is do we explore by going any way the wind blows or any way our brains blow? Here we show that, while both types of noise are present that it is noise in our brains that dominates random exploration. This suggests that decision noise for exploration is subject to cognitive control.
	...
	Using a simple gambling task, known as the Horizon Task, we have recently shown that humans appear to actually use random exploration and actively adapt their decision noise to solve simple explore-exploit problems \citep{wilson2014}.  The key manipulation in the Horizon Task is the horizon condition, i.e. the number of decisions remaining for the participant to make. Increasing the horizon makes exploration more valuable as there is more time to use the information gained by exploration to maximize future rewards. For example, if you are leaving town tomorrow (short horizon), you will probably exploit the restaurant you know and love, but if you are in town for a while (long horizon), you would be more likely to explore the new restaurant. Using such a horizon manipulation we found that people's behavior is more variable in long horizons than short horizons, suggesting that they use adaptive decision noise to solve the explore-exploit dilemma. 
	...
	%internally, from random processes in the brain, or externally, in stochastic input from the world. In the restaurant case, internal noise would arise from stochastic neural processes tossing a metaphorical coin in your head.
	%$Of these, internal noise is the simplest to 
	%externally, in the input from the world, or is generated internally, within the brain. 
	...
	%If random exploration is driven by external noise, this implies that people adapt their susceptibility to random external with horizon. Thus
	%Exploration driven by external noise, 
	...
	\begin{figure}[H]
		\begin{center}
			...
			\caption{
			%BOB: DO THE YTICKS ON THE 0.1S NOT 0.15S!  MORE WHITE SPACE BETWEEN PANELS A AND B.
			Replication of previous findings. Both  $p(\mbox{low mean})$ (A) and $p(\mbox{high info})$ (B) increase with horizon suggesting that people use both random and directed exploration in this task.  }%BOB: SWITCH A AND B TO HAVE RANDOM FIRST}
			\label{fig:modelfree}
		\end{center}
	\end{figure}
...
%	\end{figure}
	...
	%SIYU: SORRY, WHAT DO YOU MEAN BY POPING THESE INTO A SEPARATE DOCUMENT, DO YOU MEAN I START A NEW LATEX DOC AND PUT THE SUPPLEMENTARY GRAPHS THERE, DO I STILL KEEP THE FIGURES HERE
	%BOB: SUPPLEMENTARY FIGURES SHOULD START NUMBERING FROM 1.  SO Supplementary Figure 1 etc ... Pop these into a separate document for sharing.  To do refs just hard code numbers.
...
%	\begin{figure}[H]
%		\begin{center}
%			\includegraphics[width=\textwidth]{figures/taskfigb_final.pdf}
%			\caption{BOB: DONT NEED THIS FIGURE IF YOU UPDATE EARLIER TASKFIGURE. Time line of a horizon 6 game.  The game begins with four forced choice trials in which participants are instructed which option to play.   After these forced trials they are free to choose between the two options for the rest of the game (6 trials in this case).
			%BOB: THIS FONT SIZE IS GOOD - MAYBE A LITTLE BIGGER FOR TEXT
...
%	\end{figure}
	...
	\begin{figure}[H]
		\begin{center}
			...
			\caption{Parameter recovery over the subject-level means of information bonus, $A$, spatial bias, $b$, internal noise variance, $\sigma_{int}$, and external noise variance, $\sigma_{ext}$, for horizon 1 (left column) and horizon 6 (right column) games. %for horizon 1 games
			...
			%BOB: BIT MORE SPACE BETWEEN ROWS. OTHERWISE GREAT!
			}
