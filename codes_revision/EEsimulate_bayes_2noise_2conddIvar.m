function out = EEsimulate_bayes_2noise_2conddIvar(data, As, bs, nrans, ndets)
    ...
    for si = 1:data.nSubject
        ...
        nT = data.nTrial(si);
        dR = data.dR(si,1:nT)';
        dIvar = data.dIvar(si,1:nT)';
        repeatID = data.repeatID(si,1:nT)';
        horizon = data.horizon(si,1:nT)';
        infocond = data.infocond(si, 1:nT)';
        n_det = NaN(1, data.nrepeatID(si));
        for gi = 1:nT
            if nrans(horizon(gi), infocond(gi), si) > 0
                pdni = makedist('Logistic',0, nrans(horizon(gi), infocond(gi), si));
                dni = random(pdni);
            else
                dni = 0;
            end
            if isnan(n_det(repeatID(gi)))
                if ndets(horizon(gi), infocond(gi), si) > 0
                    pdne = makedist('Logistic',0, ndets(horizon(gi), infocond(gi), si));
            ...
            dQ = dR(gi) + As(horizon(gi), infocond(gi), si) * dIvar(gi) + bs(horizon(gi), infocond(gi), si) + dne + dni;
            if dQ > 0
                choice(gi) = 1;
            else
                choice(gi) = 0;
